...
class Encoder(nn.Module):
    def __init__(self, vis, channel_num, num_layers=4):
        ...
        # self.polarization_prompt_block = PolarizationPromptBlock()
    ...
    def forward(self, emb1,emb2,emb3,emb4):  #,polar
        # polarization_info=polar
        attn_weights = []
        # emb1, emb2, emb3, emb4, prompt = self.polarization_prompt_block(
        #     emb1, emb2, emb3, emb4, polarization_info)
...
class ChannelTransformer(nn.Module):
    def __init__(self,  vis=False, img_size=[256,256], channel_num=[64, 128, 256, 512], patchSize=[32, 16, 8, 4],dropout_rate=0.3):
        super().__init__()
    ...
    def forward(self,en1,en2,en3,en4): #,p
        ...
        #
        # encoded1, encoded2, encoded3, encoded4, attn_weights = self.encoder(emb1,emb2,emb3,emb4 ) # ,p # (B, n_patch, hidden)
        ...
        encoded1, encoded2, encoded3, encoded4, attn_weights = self.encoder(emb1,emb2,emb3,emb4) # ,p  # (B, n_patch, hidden)
        # Apply dropout to encoded features before reconstruction
        encoded1 = self.dropout(encoded1) if en1 is not None else None
        encoded2 = self.dropout(encoded2) if en2 is not None else None
        encoded3 = self.dropout(encoded3) if en3 is not None else None
        encoded4 = self.dropout(encoded4) if en4 is not None else None
        ...
        # Adding skip connections with dropout before adding
        x1 = self.dropout(x1 + en1) if en1 is not None else None
        x2 = self.dropout(x2 + en2) if en2 is not None else None
        x3 = self.dropout(x3 + en3) if en3 is not None else None
        x4 = self.dropout(x4 + en4) if en4 is not None else None
