...
class lab_Loss(nn.Module):
    ...
    def loss_ab(self,img,gt,alpha,tab,levels,device):
        p= self.Hist_2_Dist_AB(img, tab,alpha).to(device)
        q= self.Hist_2_Dist_AB(gt,tab,alpha).to(device)
    ...
    def forward(self,img,gt,device):

	    tab=quantAB(self.levels,self.vmin,self.vmax).to(device)
	    ...
	    loss_AB=self.loss_ab(lab_img[:,1:,:,:],lab_gt[:,1:,:,:],self.alpha,tab,self.levels,device)
